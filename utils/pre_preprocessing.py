...
#import cPickle
#
#with open('./subjects_conn_matrices.pkl','rb') as f:
#    sub_conn_matrices = cPickle.load(f)
#    
#    
#corr = stacked_matrices[groupes[1]]['correlation'][0,:,:]
#tol = 1e-7
#import numpy as np
#
#
...
#
#test = fisher_transform(corr)
#
#symmetric_array = corr
#
##Fetch diagonal of the symmetric_array for reconstruction after vectorization:
#diag_array = np.diag(symmetric_array)
#
##Discarding diagonal which is one for correlation, and partial correlation, not defined in Z-Fisher transformation
#vectorize_array = sym_matrix_to_vec(symmetric = symmetric_array, discard_diagonal=False)
#
...
#
#
##Reconstruct the symetric matrix:
#fisher_transform_array = vec_to_sym_matrix(vec = fisher_vectorize_array, diagonal = None)