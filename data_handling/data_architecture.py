...
from utils import folders_and_files_management as ffm
...
def fetch_fmri_data(root_fmri_data_directory, groupes):
    ...
    """
    #We fecth all fmri data thank to list_fmri_data
    ...
    for i in range(len(groupes)):
        if groupes[i] not in fmri_files_dict.keys():
            raise ValueError('Le groupe {} n\'existe pas !. Les groupes dans {} sont {}'.format(groupes[i],
...
def create_group_dictionnary(subjects_id_data_path, root_fmri_data_directory, groupes):
    ...
    for groupe in groupes:
        #liste des images fonctionelles
        a = glob.glob(os.path.join(root_fmri_data_directory, groupe,'*'))
        #j'extrais les identifiant de chaque element de a
        b= [re.search(s_id,f_name) for f_name in a for s_id in subjects_ID[0]]
        #Les identifiant des sujets présent dans groupe
...
def fetch_data_with_individual_atlases(subjects_id_data_path, root_fmri_data_directory, groupes, individual_atlases_directory,
                             individual_atlases_labels_directory, individual_atlas_file_extension, individual_atlas_labels_extension,
    ...
    output : dict
        ...
        IDs. The last level, is all the relevantfile for one subjects: fmri image, 
        subject atlas, subject atlas labels file, and counfound file if reguired.
    ...
    group_fmri_dict = fetch_fmri_data(root_fmri_data_directory = root_fmri_data_directory, groupes = groupes)

    subjects_ID = read_text_data_file(file_path = subjects_id_data_path, colname = None, header = None)
    
    #Full path to individual atlases files and labels file directory
    individual_atlases_files = glob.glob(os.path.join(individual_atlases_directory,individual_atlas_file_extension))
    individual_atlases_labels_files = glob.glob(os.path.join(individual_atlases_labels_directory, individual_atlas_labels_extension))
    
    #Check if counfounds files is reguired
    ...
    for s_id in subjects_ID[0]:
        for groupe in range(n_groupes):
            ...
            subject_functional_file = [ m.group(0) for l in group_fmri_dict[groupes[groupe]] for m in [regex.search(l)] if m ]
            #If fmri exist, fetch the corresponding atlas file, and atlas labels files
            if subject_functional_file != []:
                #Subject atlas
                subject_atlas_file = [ m.group(0) for l in individual_atlases_files for m in [regex.search(l)] if m  ]
                #Subject atlas labels
                subject_atlas_labels_file = [ m.group(0) for l in individual_atlases_labels_files for m in [regex.search(l)] if m  ]
                
                #Append confound if they exist:
                if individual_confounds_files:
                    subject_counfounds_file = [ m.group(0) for l in individual_confounds_files for m in [regex.search(l)] if m  ]
                    
                else:
                    subject_counfounds_file = []
                
                #Creation of a sub-dictionary for each subject containing the full path to files as values, and file descriptions as keys
                subject_files = {'functional_file': subject_functional_file, 'atlas_file': subject_atlas_file, 'label_file': subject_atlas_labels_file, 'counfounds_file': subject_counfounds_file}
    ...
    dict2 = create_group_dictionnary(subjects_id_data_path = subjects_id_data_path, root_fmri_data_directory = root_fmri_data_directory, groupes = groupes)
...
def fetch_data(subjects_id_data_path, root_fmri_data_directory, groupes,individual_counfounds_directory = None):
    ...
    output : dict
        ...
        The first level is the groups keys. The second levels is the subjects 
        IDs. The last level, is all the relevantfile for one subjects: fmri image, 
        and counfound file if reguired.
    
    See Also  
    --------
    
    fetch_data_with_individual_atlases : Fetch a complete organised structure for a groups study
    which reguire an individual atlas per subjects.
    ...
    for s_id in subjects_ID[0]:
        for groupe in range(n_groupes):
            ...
            #Fetch of fmri image
            subject_functional_file = [ m.group(0) for l in group_fmri_dict[groupes[groupe]] for m in [regex.search(l)] if m ]
            #If fmri exist we look for a counfound file
            if subject_functional_file != []:                
                #If a counfound file we append it:
                if individual_confounds_files:
                    subject_counfounds_file = [ m.group(0) for l in individual_confounds_files for m in [regex.search(l)] if m  ]
                else:
                    subject_counfounds_file = []
                ...
                #Creation of a sub-dictionary for each subject containing the full path to files as values, and file descriptions as keys
                subject_files = {'functional_file': subject_functional_file, 'counfounds_file': subject_counfounds_file}
